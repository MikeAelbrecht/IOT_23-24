...
class NRF24:
    ...
    def __init__(
        ...
        # SPI Channel
        assert (
            spi_channel >= SPI_CHANNEL.MAIN_CE0 and spi_channel <= SPI_CHANNEL.AUX_CE2
        )
        ...
        if spi_channel < SPI_CHANNEL.AUX_CE0:  # WAS: NRF24.SPI_AUX_CE0:
            ...
            # Aux SPI.
            self._spi_handle = pi.spi_open(
                spi_channel - SPI_CHANNEL.AUX_CE0, int(spi_speed), NRF24._AUX_SPI
            )
    ...
    def set_address_bytes(self, address_bytes):
        assert (
            3 <= address_bytes <= 5
        ), "Number of address bytes must be between 3 and 5."
    ...
    def get_data_rate(self):
        ...
        rate = ((rf_setup & NRF24.RF_DR_LOW) >> 4) & (
            (rf_setup & NRF24.RF_DR_HIGH) >> 3
        )
    ...
    def ack_payload(self, pipe, data):
        ...
        if pipe < NRF24.RX_ADDR_P0 or pipe > NRF24.RX_ADDR_P5:
            raise ValueError(
                f"pipe out of range ({NRF24.RX_ADDR_P0:02x} <= pipe <= and {NRF24.RX_ADDR_P5:02x})."
            )

        self._nrf_command(
            [self.W_ACK_PAYLOAD | ((pipe - RF24_RX_ADDR.P0) & 0x07)] + data
        )
        ...
        if isinstance(address, str):
            ...
            for n in address:
                if not (isinstance(n, int) and (0 <= n < 256)):
                    raise ValueError(
                        f"Invalid value {n} in address. All values must be between 0 and 256."
                    )
            addr = address
    ...
    def open_writing_pipe(self, address, size=None):
        ...
        assert (
            len(addr) == self._address_width
        ), f"Invalid address length {len(addr)} of address {address} ({addr})."

        en_rxaddr = self._nrf_read_reg(NRF24.EN_RXADDR, 1)[
            0
        ]  # Get currenly enabled addresses.
        en_aa = self._nrf_read_reg(NRF24.EN_AA, 1)[
            0
        ...
        self._open_reading_pipe(
            RF24_RX_ADDR.P0, addr, size
        )  # Open P0 for reading the acknowledgement.
    ...
    def _open_reading_pipe(self, pipe, address, size=None):
        ...
        self._nrf_write_reg(
            NRF24.EN_RXADDR, en_rxaddr | enable
    ...
    def close_reading_pipe(self, pipe):
        ...
        assert (
            0 <= pipe <= 5
        ), "Pipe should be in range 0..5 or RF24_RX_ADDR.P0..RF24_RX_ADDR.P5."
